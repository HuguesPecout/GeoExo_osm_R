...
class(Mosquee_Touba_sf)
mf_map(Mosquee_Touba_sf)
...
mat_eucli_m <- st_distance(x = Mosquee_Touba_sf, y = dep_pt) 

# Changement des noms de ligne et de colonne
rownames(mat_eucli_m) <- Mosquee_Touba_sf$name
colnames(mat_eucli_m) <- dep_pt$NAME_2
...
# mètres -> kilomètres
dep_pt$GMT_eucli_dist_km <- as.numeric(mat_eucli_m) / 1000
# mètres -> kilomètres
dep_pt$GMT_route_km <- as.numeric(dist$distances) / 1000
# Minutes -> heures
dep_pt$GMT_route_hr <- as.numeric(dist$durations) / 60
...
mean(dep_pt$GMT_eucli_dist_km)
max(dep_pt$GMT_eucli_dist_km)

mean(dep_pt$GMT_route_km)
max(dep_pt$GMT_route_km)

mean(dep_pt$GMT_route_hr)
max(dep_pt$GMT_route_hr)
...
# Indice de sinuosité 
dep_pt$ind_sinuo <- round(dep_pt$GMT_route_km / dep_pt$GMT_eucli_dist_km, 2)
# Indice de vitesse sur route
dep_pt$ind_speed <- round(dep_pt$GMT_route_km / dep_pt$GMT_route_hr, 1)
...
## H.1 Récupération de l’itinéraire “Mosquee Touba - Dakar”
...
# Titre
mf_title("Itinéraire le plus performant depuis la Grande Mosquée de Touba, 2014", 
         fg = "white")
# Sources
mf_credits(paste0(get_credit("OpenStreetMap"), " - OSRM, 2014"), 
           cex = 0.8)
